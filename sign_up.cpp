...
void Sign_up::on_sureButton_clicked()
{
    ...
    if(passwd!=passwdcon){
        QMessageBox::warning(this,"warning","两遍密码输入不一样，请重新设置密码！");
    }
    else
    {
        if(name!=nullptr&&identitynumber!=nullptr&&telephonenumber!=nullptr&&passwd!=nullptr)
        {
            QFile file(name);
            file.open(QIODevice::WriteOnly);
            QByteArray ba;
            ba.append(name);ba.append(d);
            ba.append(identitynumber);ba.append(d);
            ba.append(telephonenumber);ba.append(d);
            ba.append(passwd);
